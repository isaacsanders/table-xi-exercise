describe "orderFor", ->
  it "returns 1 dollar worth of dishes, if that is what you want", ->
    ...
    expect(menu.orderFor(1)).toContain(pop)
  ...
  it "returns 5 dollar worth of dishes, if that is what you want", ->
    ...
    expect(menu.orderFor(5)).toContain(pop)

  it "return a fractional answer", ->
    menu = new TableXIExercise.Collections.Menu()
    water = { name: "water", cost: 2.34 }
    pop = { name: "pop", cost: 3.45 }
    menu.add([water, pop])
    ...
    menu = new TableXIExercise.Collections.Menu()
    water = { name: "water", cost: 1.23 }
    pop = { name: "pop", cost: 2.34 }
    lemonade = { name: "lemonade", cost: 3.45 }
    ...
    menu = new TableXIExercise.Collections.Menu()
    water = { name: "water", cost: 12.34 }
    pop = { name: "pop", cost: 34.56 }
    menu.add([water, pop])

    expect(menu.orderFor(46.9)).toContain(water)
    expect(menu.orderFor(46.9)).toContain(pop)
